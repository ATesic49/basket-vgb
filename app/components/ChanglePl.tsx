...
function ChanglePl({ slika, setIsChanging, ime, nadimak, cena, clickable }: { clickable: boolean, slika: string, ime: string, nadimak: string, cena: number, setIsChanging: (arg: boolean) => void }) {
    return (
        <div className='p-2 flex justify-start w-full  gap-4 text-gray-600 text-sm items-center relative pt-4'>
            ...
            <div className='flex  ml-auto  text-orange-500 font-bold text-sm flex-col justify-center gap-2 items-center'>
                {
                    clickable ? <div className='flex justify-center items-center  p-2 text-xs font-bold gap-2  text-gray-100 bg-orange-500 rounded-md shadow-sm cursor-pointer' onClick={() => setIsChanging(false)} >
