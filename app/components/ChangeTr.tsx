...
function ChangeTr({ id, setIsChanging, isChanging, cena }: { id: number, cena: number, isChanging: boolean, setIsChanging: (arg: boolean) => void }) {
                    ...
                    {trainers.map((trainer, index) => {
                        ...
                        if (trainer.ime.toLowerCase().includes(input.toLowerCase()) || trainer.nadimak.toLowerCase().includes(input.toLowerCase())) {
                            if (trainer.xena! <= credits + cena) {
                                return <ChanglePl trener naTerenuId={id} id={trainer.id} clickable={true} key={index} slika={trainer.slika} ime={trainer.ime} nadimak={trainer.nadimak} cena={trainer.xena} setIsChanging={setIsChanging} />
                            } else {
                                return <ChanglePl trener naTerenuId={id} id={trainer.id} clickable={false} key={index} slika={trainer.slika} ime={trainer.ime} nadimak={trainer.nadimak} cena={trainer.xena} setIsChanging={setIsChanging} />
                            }
